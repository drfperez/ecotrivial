...
let questionBank = JSON.parse(JSON.stringify(DEFAULT_QUESTION_BANK));
...
const state = {
  ...
  editor:{ activeCategory:'verd', selectedIndex:null }
};
...
function normalizeBankShape(bank){
  const out = { verd:[], blau:[], taronja:[], vermell:[], groc:[], lila:[] };
  CATEGORY_IDS.forEach(cat => {
    out[cat] = Array.isArray(bank?.[cat]) ? bank[cat].map(q => ({
      pregunta:String(q.pregunta || '').trim(),
      opcions:Array.isArray(q.opcions) ? q.opcions.map(o => String(o || '').trim()) : [],
      correcte:Number(q.correcte),
      explicacio:String(q.explicacio || '').trim()
    })).filter(q => q.pregunta && q.opcions.length===4 && [0,1,2,3].includes(q.correcte)) : [];
  });
  return out;
}
...
function createTemplateJSON(){
  return JSON.stringify([
    { categoria:'verd', pregunta:'Què és un cultiu de cobertura?', opcions:['Una planta que protegeix el sòl','Un plàstic negre','Un tipus de reg','Una màquina'], correcte:0, explicacio:'Protegeix el sòl entre collites i en millora la fertilitat.' },
    { categoria:'blau', pregunta:'Per què la matèria orgànica ajuda amb l’aigua?', opcions:['Perquè actua com una esponja','Perquè impermeabilitza','Perquè elimina arrels','Perquè no té efecte'], correcte:0, explicacio:'Millora la capacitat de retenció d’aigua del sòl.' }
  ], null, 2);
}
...
function bankToJSONArray(){
  const arr=[];
  CATEGORY_IDS.forEach(cat => questionBank[cat].forEach(q => arr.push({
    categoria:cat, pregunta:q.pregunta, opcions:[...q.opcions], correcte:q.correcte, explicacio:q.explicacio
  })));
  return arr;
}
...
function bankToCSV(){
  ...
  CATEGORY_IDS.forEach(cat => questionBank[cat].forEach(q => {
    lines.push([esc(cat), esc(q.pregunta), esc(q.opcions[0]), esc(q.opcions[1]), esc(q.opcions[2]), esc(q.opcions[3]), esc(q.correcte), esc(q.explicacio)].join(';'));
  }));
  return lines.join('\n');
}
...
function validateRecord(obj, idx='registre'){
  ...
  if(!CATEGORY_IDS.includes(categoria)) throw new Error(`${idx}: categoria no vàlida (${categoria})`);
  ...
  const finalOpcions = opcions.map(o => String(o ?? '').trim()).filter(Boolean);
  const correcte = Number(obj.correcte ?? obj.correct ?? obj.correctIndex);

  if(!pregunta) throw new Error(`${idx}: falta la pregunta`);
  if(finalOpcions.length !== 4) throw new Error(`${idx}: calen 4 opcions`);
  if(![0,1,2,3].includes(correcte)) throw new Error(`${idx}: la resposta correcta ha de ser 0,1,2 o 3`);

  return { categoria, registre:{ pregunta, opcions:finalOpcions, correcte, explicacio } };
}

function normalizeImportedData(data){
  const bank = { verd:[], blau:[], taronja:[], vermell:[], groc:[], lila:[] };

  if(Array.isArray(data)){
    data.forEach((item, idx) => {
      const { categoria, registre } = validateRecord(item, `Fila ${idx+1}`);
      bank[categoria].push(registre);
    });
    return bank;
  }
    ...
    CATEGORY_IDS.forEach(cat => {
      if(Array.isArray(data[cat])){
        ...
        data[cat].forEach((item, idx) => {
          const { registre } = validateRecord({ ...item, categoria:cat }, `${cat} #${idx+1}`);
          bank[cat].push(registre);
        });
      }
    });
...
function persistBank(){
  localStorage.setItem('ecotrivial-editor-bank-corregit', JSON.stringify(questionBank));
}

function loadQuestionBank(){
  try{
    const saved = localStorage.getItem('ecotrivial-editor-bank-corregit');
    if(!saved){
...
function getQuestion(catId){
  ...
  const available = arr.map((q, i) => ({ q, i })).filter(item => !used.includes(item.i));
  const picked = available[Math.floor(Math.random() * available.length)];
  state.usedQuestions[catId].push(picked.i);
  return picked.q;
}
...
function saveLastConfig(){
  localStorage.setItem('ecotrivial-editor-config-corregit', JSON.stringify({
...
function loadLastConfig(){
  try{
    const saved=JSON.parse(localStorage.getItem('ecotrivial-editor-config-corregit') || 'null');
    if(!saved) return;
...
function openQuestion(team, catId, special){
  ...
  ['1','2','3','4'].forEach((lab, idx) => {
    const btn=document.createElement('button');
    btn.className='answer';
    btn.innerHTML=`<strong>${lab}.</strong> ${q.opcions[idx]}`;
...
function answerQuestion(selectedIndex, timeout=false){
  ...
  const correctIndex=q.correcte;
  const isCorrect=!timeout && selectedIndex===correctIndex;

  buttons.forEach((btn, idx) => {
    if(idx===correctIndex) btn.classList.add('correcte');
    if(!timeout && idx===selectedIndex && idx!==correctIndex) btn.classList.add('incorrecte');
  });
  ...
  if(isCorrect){
    ...
    feedbackText.innerHTML=`${q.explicacio}<br><br><strong>La resposta correcta era:</strong> ${q.opcions[correctIndex]}.<br><strong>- ${penalty} punts.</strong>`;
    setStatus(timeout
...
function renderCategoryTabs(){
  ...
  CATEGORIES.forEach(cat => {
    ...
    tab.addEventListener('click', () => {
      ...
      state.editor.selectedIndex=null;
      renderEditor();
...
function questionPreview(catId, idx){
  const q=questionBank[catId][idx];
  return `<div class="q-card ${state.editor.activeCategory===catId && state.editor.selectedIndex===idx ? 'active' : ''}" data-index="${idx}"><div class="q-title">${idx+1}. ${escapeHtml(q.pregunta)}</div><div class="q-meta">Correcta: opció ${q.correcte+1} · ${q.explicacio ? 'amb explicació' : 'sense explicació'}</div></div>`;
}

function renderQuestionList(){
  const cat=state.editor.activeCategory;
  questionList.innerHTML = (questionBank[cat] || []).map((_, idx) => questionPreview(cat, idx)).join('') || '<div class="status-line">Encara no hi ha preguntes en aquesta categoria.</div>';
  const catObj=CATEGORIES.find(c => c.id===cat);
  editorCount.textContent = `${catObj.icon} ${catObj.label} · ${(questionBank[cat] || []).length} preguntes`;
  [...questionList.querySelectorAll('.q-card')].forEach(el => el.addEventListener('click', () => loadQuestionIntoForm(Number(el.dataset.index))));
}
...
function loadQuestionIntoForm(index){
  const cat=state.editor.activeCategory;
  const q=questionBank[cat][index];
  if(!q) return;

  state.editor.selectedIndex=index;
  editorCategory.value=cat;
  editorCorrect.value=String(q.correcte);
  editorQuestion.value=q.pregunta;
  editorOption1.value=q.opcions[0] || '';
  editorOption2.value=q.opcions[1] || '';
  editorOption3.value=q.opcions[2] || '';
  editorOption4.value=q.opcions[3] || '';
  editorExplanation.value=q.explicacio || '';
  editorStatus.textContent=`Editant la pregunta ${index+1} de la categoria ${cat}.`;
...
function getFormQuestion(){
  return {
    categoria:editorCategory.value,
    pregunta:editorQuestion.value.trim(),
    opcions:[editorOption1.value.trim(), editorOption2.value.trim(), editorOption3.value.trim(), editorOption4.value.trim()],
    correcte:Number(editorCorrect.value),
    explicacio:editorExplanation.value.trim()
  };
}

function saveQuestionFromForm(){
  try{
    const { categoria, registre } = validateRecord(getFormQuestion(), 'Formulari');

    if(categoria !== state.editor.activeCategory){
      state.editor.activeCategory = categoria;
      state.editor.selectedIndex = null;
    }

    if(state.editor.selectedIndex === null){
      questionBank[categoria].push(registre);
      state.editor.selectedIndex = questionBank[categoria].length - 1;
      editorStatus.textContent='Pregunta nova desada correctament.';
    } else {
      questionBank[categoria][state.editor.selectedIndex] = registre;
    ...
    updateBankStatus('custom');
    renderEditor();
    loadQuestionIntoForm(state.editor.selectedIndex);
  } catch(err){
...
function duplicateQuestion(){
  ...
  const clone=JSON.parse(JSON.stringify(questionBank[cat][idx]));
  clone.pregunta += ' (còpia)';
  questionBank[cat].splice(idx+1, 0, clone);

  persistBank();
  updateBankStatus('custom');
  renderEditor();
  loadQuestionIntoForm(idx+1);
...
function renderEditor(){
  renderCategoryTabs();
  renderQuestionList();
  if(state.editor.selectedIndex===null || !(questionBank[state.editor.activeCategory][state.editor.selectedIndex])) {
    startNewQuestion(state.editor.activeCategory);
  }
}
...
btnExportJson.addEventListener('click', () => downloadTextFile('ecotrivial_preguntes.json', JSON.stringify(bankToJSONArray(), null, 2), 'application/json;charset=utf-8'));
...
btnEditorExportJson.addEventListener('click', () => downloadTextFile('ecotrivial_preguntes.json', JSON.stringify(bankToJSONArray(), null, 2), 'application/json;charset=utf-8'));
...
if(!localStorage.getItem('ecotrivial-help-center-seen')){
  setTimeout(() => {
    openHelpCenter('play');
    localStorage.setItem('ecotrivial-help-center-seen', '1');
  }, 500);
}
